...
import type { OSCMessageInterface } from '../index'
import type { OSCArgTypeNest } from '../x32'

const SHOW_MODES = ['CUES', 'SCENES', 'SNIPPETS']

const getIndex  = ( x : string ) => parseInt( x.slice( x.lastIndexOf( '/' )+1 ), 10 )
const getCueNum = ( x : string ) => `${x.slice( 0, x.length-2 )}.${x.slice( -2, -1 )}.${x.slice( -1 )}`

export const dB2Float = ( db_in : string | number ) => {
	let d : number
...
export const float2dB = ( f : number ) => {
	if ( typeof f !== 'number' ) {
		throw new TypeError( 'float expected' )
	}
...
const normalizeLevel = ( level : number | string, fader : string ) : Record<string, OSCArgTypeNest> => ( {
...
const normalizeMute = ( mute : string | number, fader : string ) : Record<string, OSCArgTypeNest> => ( {
	index   : parseInt( fader ),
	isOn    : {
...
const normalizeMix = ( mute : string | number, level : string | number, fader : string ) : Record<string, OSCArgTypeNest> => ( {
	...normalizeMute( mute, fader ),
	...normalizeLevel( level, fader ),
} )

const normalizeName = ( faderName : string, fader : string ) : Record<string, OSCArgTypeNest> => ( {
	index   : parseInt( fader ),
	name    : faderName.toString(),
	zIndex  : fader,
} )

const normalizeShowMode = ( value : string | number ) : Record<string, OSCArgTypeNest> => ( {
	index   : ( typeof value === 'number' ) ? value : SHOW_MODES.indexOf( value ),
	name    : ( typeof value === 'string' ) ? value : SHOW_MODES[value] || 'error',
} )

type testProps = ( msgObj : OSCMessageInterface ) => Record<string, OSCArgTypeNest>

type testObject = { regEx : RegExp, props : testProps }

export const regular : Record<string, testObject> = {
	showCurrent : {
		regEx : /^\/-show\/prepos\/current$/,
		props : ( msgObj : OSCMessageInterface ) => ( {
	...
	showMode : {
		...
		props : ( msgObj : OSCMessageInterface ) =>
			normalizeShowMode( msgObj.args[0]!.value as string | number ),
	},

	auxLevel : {
		regEx : /^\/auxin\/\d{2}\/mix\/fader$/,
		props : ( msgObj : OSCMessageInterface ) => normalizeLevel(
			msgObj.args[0]!.value as string | number,
			msgObj.address.slice( 7, 9 )
		),
	},
	auxMute : {
		regEx : /^\/auxin\/\d{2}\/mix\/on$/,
		props : ( msgObj : OSCMessageInterface ) => normalizeMute(
			msgObj.args[0]!.value as string | number,
			msgObj.address.slice( 7, 9 )
		),
	},
	auxName : {
		...
		props : ( msgObj  : OSCMessageInterface ) => normalizeName(
			msgObj.args[0]!.value as string,
			msgObj.address.slice( 7, 9 )
		),
	},
...
export const node : Record<string, testObject> = {
	auxMix : {
		regEx : /^\/auxin\/\d{2}\/mix$/,
		props : ( msgObj  : OSCMessageInterface ) => normalizeMix(
			msgObj.args[0]!.value as string | number,
			msgObj.args[1]!.value as string | number,
			msgObj.address.slice( 7, 9 )
		),
	},
	auxName : {
	...
	showMode : {
		regEx : /^\/-prefs\/show_control$/,
		props : ( msgObj  : OSCMessageInterface ) => normalizeShowMode( msgObj.args[0]!.value as string | number ),
	},
	showName : {
	...
	showCue : {
		...
		props : ( msgObj  : OSCMessageInterface ) => ( {
			...
			cueScene   : msgObj.args[3]!.value as number,
			cueSkip    : Boolean( msgObj.args[2]!.value ),
			cueSnippet : msgObj.args[4]!.value as number,
