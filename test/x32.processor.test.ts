...
const testData : [string, OSCArguments['value'], unknown ][] = [
...
describe( 'main class operate as expected', () => {
	test.each( testData )( 'Against address %s', ( a, b, c ) => {
		const msg = oscLib.newMessage(
			a as string,
			[
				oscLib.autoType( b as OSCArguments['type'] )
			]
		)
	...
	test( 'batch processing', () => {
		const goodMessages = testData.map( ( [a, b] ) => oscLib.newMessage(
			a as string,
			[
				oscLib.autoType( b as OSCArguments['type'] )
			] )
		)
		const badMessages = nullData.map( ( [a, b] ) => oscLib.newMessage(
			a as string,
			[
				oscLib.autoType( b as OSCArguments['type'] )
			] )
		)
		const bundle = oscLib.newBundle( badMessages )
