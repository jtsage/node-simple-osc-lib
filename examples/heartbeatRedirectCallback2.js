...
const heartBeatMessage = (oscLib.messageBuilder('/hello')).integer(20)
const heartBeatBuffer  = heartBeatMessage.toBuffer()


const redirectCallback = (bufferNewAddress, bufferOldAddress, _argList, argBuffer) => {
...
oscSocket.on('message', (msg, _rinfo) => {
	try {
		const newPacket = oscLib.redirectMessage(msg, '/newTown', redirectCallback)
