...
describe( 'data read operate as expected', () => {
	test( 'fader', () => {
		...
		const msg = OSCMessage.newMessage(
			'/mtx/04/mix/fader',
			[{ type : 'float', value : 1}]
		)
		...
		if ( matches !== null ) {
			const result = engine.processor( msg, matches[0] ) as X32FaderUpdateLevel
	...
	test( 'fader (missing part, test factory)', () => {
		...
		if ( matches !== null ) {
			const result = engine.processor( msg, matches[0] )
	...
	test( 'mute', () => {
		...
		if ( matches !== null ) {
			const result = engine.processor( msg, matches[0] ) as X32FaderUpdateMute
	...
	test( 'name', () => {
		...
		if ( matches !== null ) {
			const result = engine.processor( msg, matches[0] ) as X32FaderUpdateName
	...
	test( 'node cue', () => {
		...
		if ( matches !== null ) {
			const result = engine.processor( msg, matches[0] ) as X32Cue
	...
	test( 'node scene', () => {
		...
		if ( matches !== null ) {
			const result = engine.processor( msg, matches[0] ) as X32Scene
	...
	test( 'node snippet', () => {
		...
		if ( matches !== null ) {
			const result = engine.processor( msg, matches[0] ) as X32Snippet
	...
	test( 'info : dirty', () => {
		...
		if ( matches !== null ) {
			const result = engine.processor( msg, matches[0] ) as X32Info
